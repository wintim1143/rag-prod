...
- [x] 混合检索能同时召回语义改写与精确术语两类匹配
- [x] 重排会改变粗筛顺序：在演示 query 上，重排后 top-k 与原始混合排序有实质差异
- [x] `/search` 响应暴露每阶段分数（向量/BM25/RRF/重排）
- [x] k/N 通过环境变量可调；在评测集上重排能提升 top-k 精度（可先用关键词探针快速验证）
- [x] 本地 cross-encoder 加载失败时有清晰报错并可降级到启发式兜底
